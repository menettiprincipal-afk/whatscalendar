...
2. **Google OAuth 2.0**: O usuário faz logon após inputar o WhatsApp, nos dá permissão de escopo `.readonly` (e e-mail para exibirmos no painel). Os tokens (access_token, refresh_token) são guardados no MongoDB individualmente. Após a autenticação, o usuário seleciona quais agendas (`CalendarList`) deseja monitorar, cujos IDs são salvos no campo `selectedCalendars`.
